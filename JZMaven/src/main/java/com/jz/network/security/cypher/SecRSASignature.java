...
public class SecRSASignature {
  ...
  private SecRSASignature() throws GeneralSecurityException {
    // generate key pair:
  ...
  private SecRSASignature(byte[] pk, byte[] sk) throws GeneralSecurityException {
    // create from bytes:
  ...
  private boolean verify(byte[] message, byte[] sign) throws GeneralSecurityException {
    // verify by pk:
    Signature signature = Signature.getInstance("SHA1withRSA");
