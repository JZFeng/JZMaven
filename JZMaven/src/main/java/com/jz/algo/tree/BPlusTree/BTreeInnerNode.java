...
class BTreeInnerNode<TKey extends Comparable<TKey>> extends BTreeNode<TKey> {
    ...
    private void insertAt(int index, TKey key, BTreeNode<TKey> leftChild,
    ...
    @Override
    protected BTreeNode<TKey> pushUpKey(TKey key, BTreeNode<TKey> leftChild,
    ...
    @Override
    protected void processChildrenTransfer(BTreeNode<TKey> borrower,
        ...
        if (borrowIndex == 0) {
            ...
            TKey upKey = borrower.transferFromSibling(this.getKey(borrowerChildIndex),
                    lender, borrowIndex);
            this.setKey(borrowerChildIndex, upKey);
        } else {
            // borrow a key from left sibling
            TKey upKey = borrower.transferFromSibling(
    ...
    @Override
    protected BTreeNode<TKey> processChildrenFusion(BTreeNode<TKey> leftChild,
                                                    BTreeNode<TKey> rightChild) {
        int index = 0;
    ...
    @Override
    protected TKey transferFromSibling(TKey sinkKey, BTreeNode<TKey> sibling,
                                       int borrowIndex) {
