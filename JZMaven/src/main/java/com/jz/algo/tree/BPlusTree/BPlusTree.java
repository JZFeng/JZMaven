...
public class BPlusTree {
    ...
    // Split the node, node, of a B-Tree into two nodes that contain T-1 (and T) elements and move node's median key up
    // to the parentNode.
    ...
    void splitChildNode(Node parentNode, int i, Node node) {
        ...
        newNode.mNumKeys = T;
        for (int j = 0; j < T; j++) { // Copy the last T elements of node into newNode. Keep the median key as duplicate in the first key of
            // newNode.
    ...
    public static void main(String[] args) {
        ...
        int primeNumbers[] = new int[]
                {2, 3, 5, 7, 11, 13, 19, 23, 37, 41, 43, 47, 53, 59, 67, 71, 61, 73, 79, 89, 97,
                        101, 103, 109, 29, 31, 113, 127, 131, 137, 139, 149, 151, 157, 163, 167,
                        173, 179, 17, 83, 107};
        ...
        for (int i = 0; i < primeNumbers.length; i++) {
            ...
            if (!value.equals(searchResult)) {
                System.out.println("Oops: Key " + primeNumbers[i] + " retrieved object "
                        + searchResult);
            }
        }
